...
// The proportion of the canvas area that should be filled by the
// rendered collection of tiles.
const VIEW_FILLED = 0.95;
const KITE_COLOUR = '#00ffff';
const DART_COLOUR = '#ff0000';
const OUTLINE_COLOUR = "#555555";

/* Draw a given collection of tiles about the origin, scaled so that 
 * the square [-1, 1] and [-1, 1] occupy most of the canvas area. */
export function drawTiling(canvas, context, collection, previous)
{
    ...
    let scale = VIEW_FILLED*Math.min(canvas.width, canvas.height)/2;
    let viewCenterX = 0;
    let viewCenterY = 0;
	...
	for (let n = 0; n < previous.kites.length; n++) {
	    drawTile(context, previous.kites[n], false, true);
	}
	for (let n = 0; n < previous.darts.length; n++) {
...
/* Draw a tile about the origin (0, 0) */
